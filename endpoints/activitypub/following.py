        ...
        if link:
            ...
            async with client.get(
                ...
                if isinstance(model, Actor):
                    if isinstance(model.following, str):
                        async with client.get(
                            model.following,
                            headers={
                                "Accept": "application/activity+json",
                            ...
                            following_model = await following.parse()
                            if not isinstance(following_model, OrderedCollection):
                                return JSONBadge(
                                    label="Error",
                                    message="Not Found",
                                    color="red",
                    ...
                    elif isinstance(model.following, OrderedCollection):
                        following_model = model.following
                    else:
                        return JSONBadge(
                            label="Error",
                            message="Not Found",
                    ...
                    if isinstance(following_model.totalItems, int):
                        return JSONBadge(
                            label="Following",
                            message=f"{following_model.totalItems}",
                            color="FA007C",
                            isError=False,
